...
async fn handle_command(
    ...
    // `resolve_for_command` is async (#7140: the conversation override is a
    // store lookup), so this has to be an async closure too — upstream's sync
    // form predates that change.
    let reset_targets = || async {
        let mut targets: Vec<AgentId> = Vec::new();
        if let Some(agent_id) = resolve_for_command().await {
    ...
    match name {
        ...
        "new" => {
            ...
            let targets = reset_targets().await;
            if targets.is_empty() {
...
mod tests {
    ...
    async fn new_resets_every_broadcast_target() {
        ...
        let reply = handle_command(
            ...
            None,
            "vip",
            &Arc::new(crate::thread_ownership::ThreadOwnershipRegistry::new()),
        )
        .await;
