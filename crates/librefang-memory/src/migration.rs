...
pub fn run_migrations(conn: &Connection) -> Result<(), rusqlite::Error> {
    ...
    // v57: agent manifest version history so operators can see how an
...
/// v57: agent manifest version history.
///
...
fn migrate_v57(conn: &Connection) -> Result<(), rusqlite::Error> {
    ...
    conn.execute(
        "INSERT OR IGNORE INTO migrations (version, applied_at, description) \
         VALUES (57, datetime('now'), 'Agent manifest version history table')",
        [],
    )?;
    Ok(())
}
