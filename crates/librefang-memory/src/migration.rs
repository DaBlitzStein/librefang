...
pub fn run_migrations(conn: &Connection) -> Result<(), rusqlite::Error> {
    ...
    // v55 (#7808): add the `memories_fts` FTS5 index plus the triggers that
    ...
    //
    // Upstream numbered this 50, which this ladder already spent on
    // `sessions.parent_session_id` (#7752). Same reason as the v52..v54
    // splice above: a database already reporting 51+ would skip a step
    // renumbered below its `user_version` and come up without the index, so
    // upstream's step is appended above this ladder's high-water mark.
...
/// v56 (#7912): Record which embedding model produced each stored vector.
///
...
fn migrate_v56(conn: &Connection) -> Result<(), rusqlite::Error> {
    ...
    conn.execute(
        "INSERT OR IGNORE INTO migrations (version, applied_at, description) \
         VALUES (56, datetime('now'), 'Add memories.embedding_model so a stored vector records which model produced it (#7912)')",
        [],
    )?;
    Ok(())
}
...
/// of double-inserting.
///
/// Upstream numbered this 50; see the ladder in `run_migrations` for why it is
/// appended here instead.
fn migrate_v55(conn: &Connection) -> Result<(), rusqlite::Error> {
    ...
    conn.execute(
        "INSERT OR IGNORE INTO migrations (version, applied_at, description) \
         VALUES (55, datetime('now'), 'Add memories_fts FTS5 index over memories.content so search without embeddings is a real index, not a LIKE scan (#7808)')",
        [],
    )?;
    Ok(())
}
...
/// what makes [`migrate_v55`] re-runnable.
...
mod tests {
    ...
    #[test]
    fn test_migrate_v55_creates_memories_fts_and_keeps_it_in_step() {
        ...
        assert_eq!(
            after_rerun, 1,
            "re-running migrate_v55 must not duplicate index rows"
        );
    }

    #[test]
    fn test_migrate_v55_backfills_memories_written_before_the_index_existed() {
