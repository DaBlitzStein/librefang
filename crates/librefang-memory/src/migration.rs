...
pub fn run_migrations(conn: &Connection) -> Result<(), rusqlite::Error> {
    ...
    //
    // Renumbered from 61 rather than taking the number first: an installation
    // that already ran the integration build has 61 applied as the task-board
    // TTL, and moving a step a database has already recorded would leave its
    // audit row describing a schema it never applied.
    //
    // The ladder must stay contiguous rather than skipping ahead to leave room
    // for other open PRs: `run_step!` gates on `current_version < N` read once
    // at boot, so a database that reaches N via a binary with a gap below it
    // will never run the skipped migrations — the backfill at the end of
    // `run_migrations` writes their audit rows anyway, so the skew is silent
    // and permanent.
