...
mod tests {
    ...
    fn v54_marks_pre_existing_agent_rows_as_lineage_unknown() {
        ...
        // Steps 51-58 all fire from 50, so the fixture also needs the tables 51, 52, 56 and 57 alter — `memories`, `group_roster`, `task_queue` and `sessions` — even though this test asserts nothing about them. v58 needs no table of its own: it only `CREATE TABLE IF NOT EXISTS manifest_versions`.
