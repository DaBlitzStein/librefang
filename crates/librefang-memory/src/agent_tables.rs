...
pub const AGENT_SCOPED_TABLES: &[(AgentTableGroup, &str, &str)] = &[
    ...
    // manifest_versions (#8041) stores the per-agent manifest history. It is
    // listed here rather than left to the hand-written cascade it arrived in,
    // because that cascade is the thing this constant replaces: an agent-keyed
    // table missing from here is one the orphan scan cannot see and the purge
    // reports as "nothing to purge" while its rows survive.
