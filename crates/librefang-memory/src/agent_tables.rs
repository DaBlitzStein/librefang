...
pub const AGENT_SCOPED_TABLES: &[(AgentTableGroup, &str, &str)] = &[
    ...
    // manifest_versions (v56) keeps an agent's manifest history; it is
    // agent-scoped and foreign-keys nothing, so it goes before `agents` like
    // every other row that outlives the agent row otherwise.
