...
fn boot_detects_chain_break_reports_exact_seq_and_does_not_prevent_startup() {
    ...
    // Build a healthy 5-entry chain the normal way.
    {
        let log = AuditLog::with_db(pool.clone());
        for i in 0..5 {
            log.record("agent-1", AuditAction::ToolInvoke, format!("action {i}"), "ok");
        }
    }
    ...
    log.record("agent-1", AuditAction::ToolInvoke, "post-break append", "ok");
    assert_eq!(log.len(), 6);
}
...
fn diagnose_chain_finds_the_same_break_boot_verification_reports() {
    ...
    assert_eq!(break_point.to_string(), format!("chain break at seq 3: expected prev_hash {} but found {bogus}", break_point.expected));
}
...
fn reanchor_after_break_preserves_pre_break_history_and_resumes_a_verifiable_chain() {
    ...
    let outcome = {
        ...
        reanchor_after_break(&mut conn, &break_point, "operator confirmed via #7702 runbook").unwrap()
    };
    ...
    let survivors: i64 = conn
        .query_row("SELECT COUNT(*) FROM audit_entries WHERE seq < 3", [], |r| r.get(0))
        .unwrap();
    assert_eq!(survivors, 3, "pre-break history must be fully preserved");

    let marker_action: String = conn
        .query_row("SELECT action FROM audit_entries WHERE seq = 3", [], |r| r.get(0))
        .unwrap();
    ...
    log.record("agent-1", AuditAction::ToolInvoke, "post-reanchor append", "ok");
    assert_eq!(log.len(), 5);
    assert!(log.verify_integrity().is_ok());
}
...
fn reanchor_after_break_at_seq_zero_falls_back_to_genesis() {
    ...
    let prev_hash: String = conn
        .query_row("SELECT prev_hash FROM audit_entries WHERE seq = 0", [], |r| r.get(0))
        .unwrap();
    assert_eq!(prev_hash, genesis, "marker must chain from the genesis sentinel");
    assert_eq!(diagnose_chain(&conn).unwrap(), None);
}
...
fn reanchor_after_break_records_seq_and_note_in_marker_detail() {
    ...
    {
        let mut conn = pool.get().unwrap();
        reanchor_after_break(&mut conn, &break_point, "confirmed benign after proteo incident review").unwrap();
    }

    let conn = pool.get().unwrap();
    let detail: String = conn
        .query_row("SELECT detail FROM audit_entries WHERE seq = 1", [], |r| r.get(0))
        .unwrap();
