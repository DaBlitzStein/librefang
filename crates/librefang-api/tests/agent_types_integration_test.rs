...
//!   * `GET    /api/agent-types/{name}`   — get one agent type
//!   * `POST   /api/agent-types`          — create an agent type
//!   * `PUT    /api/agent-types/{name}`   — update an agent type
//!   * `DELETE /api/agent-types/{name}`   — delete an agent type
//!
...
async fn boot() -> Harness {
    ...
    let app = routes::agent_templates::agent_types_router()
...
async fn agent_types_crud_lifecycle() {
    ...
    let req = Request::builder()
        .method(Method::GET)
        .uri("/agent-types/test-agent-crud")
        .body(Body::empty())
        .unwrap();
    let resp = h.app.oneshot(req).await.unwrap();
    assert_eq!(resp.status(), StatusCode::OK);
...
async fn agent_types_create_rejects_duplicate() {
    ...
    let req = Request::builder()
        .method(Method::DELETE)
        .uri("/agent-types/duplicate-test")
        .body(Body::empty())
        .unwrap();
...
async fn agent_types_get_nonexistent_returns_404() {
    ...
    let req = Request::builder()
        .method(Method::GET)
        .uri("/agent-types/nonexistent-type")
        .body(Body::empty())
        .unwrap();
    let resp = h.app.oneshot(req).await.unwrap();
    assert_eq!(resp.status(), StatusCode::NOT_FOUND);
...
async fn agent_types_list_includes_created_items() {
    ...
    let req = Request::builder()
        .method(Method::DELETE)
        .uri("/agent-types/list-include-test")
        .body(Body::empty())
        .unwrap();
    let _ = h.app.oneshot(req).await;
}
...
async fn agent_types_escapes_toml_special_chars() {
    ...
    let req = Request::builder()
        .method(Method::GET)
        .uri("/agent-types/toml-inject-test")
        .body(Body::empty())
        .unwrap();
    let resp = h.app.oneshot(req).await.unwrap();
    assert_eq!(resp.status(), StatusCode::OK);
