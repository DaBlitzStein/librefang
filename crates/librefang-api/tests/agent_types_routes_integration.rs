...
/// The registry checkout root the diff/restore handlers resolve: `<home>/registry`,
/// with each agent type stored directory-per-type as `agent-types/{name}/agent.toml`.
/// Resolved against the harness kernel's own `home_dir`, which is what those handlers
/// read now that they take it from `AppState` rather than `LIBREFANG_HOME` (#8112).
