...
fn tool_ctx(kernel: &Arc<dyn KernelHandle>) -> ToolExecContext<'_> {
    ToolExecContext {
        ...
        // `owner` is our #7744 field: the authenticated human behind the turn.
        // A raw tool invocation in a test has no authenticated initiator, which
        // is the same `None` cron fires and agent-to-agent sends carry.
        owner: None,
        skill_registry: None,
        allowed_skills: None,
