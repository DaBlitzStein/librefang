...
/// The cadence survives the create → read round-trip, so a goal's own document
/// is what drives the loop rather than a hard-wired constant.
#[tokio::test(flavor = "multi_thread")]
...
/// A goal created without the field keeps it absent, so existing documents and
/// clients that never send it are untouched.
#[tokio::test(flavor = "multi_thread")]
...
#[tokio::test(flavor = "multi_thread")]
async fn goal_run_start_validates_a_tick_interval_override() {
    let h = boot().await;
    let agent = "11111111-1111-1111-1111-111111111111";
    let goal = create_goal(
        &h,
        serde_json::json!({"title": "Override cadence", "agent_id": agent}),
    )
    .await;
    let id = goal["id"].as_str().unwrap().to_string();
    ...
    let (status, body) = json_request(
        ...
        Method::POST,
        &format!("/api/goals/{id}/start"),
        Some(serde_json::json!({"tick_interval_secs": 60})),
    )
    .await;
