...
export function SkillsPage() {
  ...
  const handleInstall = (
    ...
    // Dedup key: `${src}:${slug}` so two skills sharing a slug on different
    // marketplaces do not collide, and the match in installPending stays
    // unambiguous (#6699). `tryStartInstall` takes the key as an opaque
    // string, so the composite one needs no change there and keeps upstream's
    // guard against a second install starting before the first resolves
    // (#7476) — the two protect different things and both are wanted.
