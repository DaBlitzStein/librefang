...
export type StepAgentBindingValue = {
  ...
  agentType: string;
  /** Spawn a new instance from the template instead of reusing a running one.
   *  Only sent when `source === "type"`, and orthogonal to `sessionMode`:
   *  this picks a different *agent*, `sessionMode` picks a different *session*
   *  on whichever agent gets resolved. Optional rather than a plain boolean so
   *  an unset binding projects to no key at all, which is what every step
   *  authored before the control existed already looks like. */
  fresh?: boolean;
...
export function bindingFromNodeData(data: CanvasNodeData): StepAgentBindingValue {
  ...
  return {
    ...
    agentName,
    agentType,
    fresh: data.fresh === true ? true : undefined,
...
> {
  ...
  if (value.source === "name") {
    ...
    return {
      ...
      agentType: undefined,
      // `fresh` describes what to do with a *template*, so it is cleared
      // alongside `agentType` rather than lingering on a binding that names
      // an agent directly.
      fresh: undefined,
      sessionMode,
      requiredSkills,
  ...
  if (value.source === "type") {
    ...
    return {
      ...
      fresh: value.fresh === true ? true : undefined,
      sessionMode,
      requiredSkills,
    };
  }
...
export function stepAgentFields(data: CanvasNodeData): {
  ...
  const payload = stepAgentPayload(data) ?? {};
  return {
    ...payload,
    // Only an `agent_type` step has anything to spawn, and only `true` is
    // worth sending: the parser's default for an absent key is already
    // find-or-reuse.
    fresh:
      "agent_type" in payload && data.fresh === true ? true : undefined,
...
export function StepAgentBinding({
  ...
  value: StepAgentBindingValue;
  agents: AgentItem[];
  /** Names of the agent types defined on this instance, offered as
   *  suggestions. Suggestions only — the box stays free text because a
   *  workflow may legitimately target a type that is authored later. */
  agentTypes?: string[];
        ...
        <>
          ...
          <label className="mt-2 flex items-center gap-2 text-[11px] text-text-dim">
            <input
              ...
              onChange={e =>
                onChange({ ...value, fresh: e.target.checked ? true : undefined })
              }
            />
