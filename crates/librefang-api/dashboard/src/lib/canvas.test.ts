...
// #6943 review: "Save as Workflow" must transform the raw workflow_create
// steps into canvas nodes/edges before the draft is handed to /canvas.
describe("workflowStepsToCanvasState", () => {
  ...
  // #6943 review — the full "Save as Workflow" chat action, composed end
  // to end: a chat message containing a workflow_create-shaped JSON blob
  // (with trailing prose that used to break the greedy extraction regex)
  // is parsed, transformed into canvas nodes/edges, and the result is fed
  // through `parseCanvasImport` — the same structural validator the
  // canvas page's own import path runs — to prove the hand-off produces
  // a well-formed canvas, not just a shape that happens to type-check.
