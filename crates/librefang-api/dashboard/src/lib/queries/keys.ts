...
export const agentKeys = {
  ...
  /** Agent-type catalog rows for the agent editor's type picker.
   *
   * Distinct from `agentTypeKeys` (the Agent Types page's own domain): this one
   * is invalidated with the rest of the agent domain, so editing an agent
   * refreshes the picker without touching the catalog page's cache. */
