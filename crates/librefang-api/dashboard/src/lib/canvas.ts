...
export type CanvasNodeData = {
  ...
  agentType?: string;
  /** Spawn a *new instance* from the bound agent type instead of reusing one
   *  that is already running. Only meaningful alongside `agentType`, and
   *  distinct from `sessionMode`: `fresh` picks a different agent, while
   *  `sessionMode` picks a different session on whichever agent is resolved.
   *  Maps to the kernel's `StepAgent::ByType { fresh }`. Present only when
   *  true — absent is the find-or-reuse default every older step carries. */
  fresh?: boolean;
