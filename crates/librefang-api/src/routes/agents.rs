...
pub async fn get_agent_channels(
    ...
    // After the sidecar migration, all channels are `[[sidecar_channels]]` entries
    // on `KernelConfig`; the old `configured_channel_names` helper (which walked
    // the in-process `ChannelsConfig` typed fields) was removed in #5461.
