...
async fn start_or_resume(
    ...
    // `resume_goal_run` reads the same loop-engineering fields back off the
    // goal document itself, so a resumed run keeps the verifier gate it was
    // started with — see `goal_lifecycle.rs::goal_run_resume`.
