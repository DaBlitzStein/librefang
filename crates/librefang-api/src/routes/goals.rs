...
pub async fn create_goal(
    ...
    // Same boundary rule as `agent_id` / `parent_id`, through the same
    // helper: blank or null clears (nothing to store on a create — #7785
    // review), a non-empty value must be a UUID, and a non-string value is a
    // 400 rather than a silently dropped field. `optional_uuid_field`
    // treating a blank string or `null` as "not set" (#6562) means a
    // verifier field left blank in a form no longer 400s as "Invalid
    // verify_agent_id" — only a non-blank, non-UUID value does.
