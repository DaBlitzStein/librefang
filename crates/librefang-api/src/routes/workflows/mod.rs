...
fn parse_step_agent(step: &serde_json::Value, step_name: &str) -> Result<StepAgent, String> {
    ...
    Ok(match key {
        ...
        // `fresh` is a modifier on the agent-type reference, not a fourth way
        // to name an agent, so it stays out of STEP_AGENT_KEYS and out of the
        // exactly-one-of rule above (#7717).
        _ => StepAgent::ByType {
