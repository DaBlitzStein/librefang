...
pub async fn get_registry_diff(
    ...
    //
    // Resolved against the same `home_dir` the write verbs and the spawn path use (#8112),
    // not the process-wide `LIBREFANG_HOME`: the diff is only meaningful against the file
    // a restore would actually overwrite.
    let home_dir = state.kernel.config_ref().home_dir.clone();
