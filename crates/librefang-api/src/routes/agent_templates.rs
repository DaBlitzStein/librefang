...
//!
//! Mounts `/profiles`, `/profiles/{name}`, `/templates`, `/templates/{name}`, `/templates/{name}/toml`, `/templates/{name}/promote`, `/templates/{name}/registry-diff`, and `/templates/{name}/restore`.
...
pub async fn put_agent_template_toml(
    ...
    // #8112: resolved against the kernel's own `home_dir`, the same one `persist_agent_type_in`
    // below writes through, so the existence check and the write cannot look at two directories.
    let home_dir = state.kernel.config_ref().home_dir.clone();
...
pub async fn post_agent_template_toml(
    ...
    // #8112: same `home_dir` the flat-shape create and the `agent_type_create` tool claim against.
    let home_dir = state.kernel.config_ref().home_dir.clone();
...
pub async fn restore_from_registry(
    ...
    //
    // This is the one snapshot in this file that is not best-effort, because it is the only copy of the content the `persist_agent_type_in` below overwrites.
