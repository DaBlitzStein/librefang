...
model-overrides-saved = { $model }의 재정의가 저장되었습니다
model-overrides-save-failed = { $model }의 재정의 저장에 실패했습니다 (상태 { $status })
model-overrides-cleared = { $model }의 재정의가 초기화되었습니다
model-overrides-clear-failed = { $model }의 재정의 초기화에 실패했습니다 (상태 { $status })
model-overrides-none-set = { $model }에 설정된 재정의가 없습니다.
...
chat-runner-goal-no-agent = 선택된 에이전트가 없습니다.
...
chat-runner-no-agent-templates = 에이전트 유형을 찾을 수 없습니다. `librefang init`을 실행하십시오.
chat-runner-invalid-template = 잘못된 에이전트 유형 '{ $name }': { $error }
...
tui-event-model-routing-update-error = 모델 라우팅 업데이트: { $error }
...
tui-event-model-overrides-fetch-failed = { $model }의 재정의를 불러오지 못했습니다
tui-event-model-overrides-save-failed = { $model }의 재정의를 저장하지 못했습니다
tui-event-model-overrides-reset-failed = { $model }의 재정의를 초기화하지 못했습니다
...
tui-mod-model-overrides-saved = { $model }의 재정의가 저장되었습니다
tui-mod-model-overrides-reset = { $model }의 재정의가 초기화되었습니다
...
tui-settings-hints-models =   [↑↓] 탐색  [e] 창 편집  [x] 초기화  [r] 새로고침
tui-settings-hints-models-edit =   [Tab] 필드  [Enter] 저장  [Esc] 취소
...
tui-settings-tab-backups = 5 백업
tui-settings-tab-capabilities = 4 기능
tui-settings-hints-capabilities =   [↑↓] 탐색  [Enter] 편집  [r] 새로고침
tui-settings-hints-capabilities-edit =   제공자/모델 입력  [Enter] 저장  [Esc] 취소  (비우면 자동 감지)
tui-settings-capabilities-header-capability = 기능
tui-settings-capabilities-header-target = 제공자 / 모델
tui-settings-capabilities-auto = 자동 감지
tui-settings-capabilities-explainer = 모든 에이전트의 전역 기본값이며, 에이전트 자체의 [capabilities]가 우선합니다.
tui-settings-capability-image-understanding = 비전
tui-settings-capability-speech-to-text = 전사
tui-settings-capability-image-generation = 이미지 생성
tui-settings-capability-text-to-speech = 음성 생성
tui-settings-capability-video-generation = 비디오 생성
tui-settings-capability-music-generation = 음악 생성
tui-event-capabilities-fetch-failed = 기능 라우팅을 불러오지 못했습니다
tui-event-capabilities-save-failed = { $capability }의 라우팅을 저장하지 못했습니다
tui-mod-capability-routing-saved = { $capability }의 라우팅을 저장했습니다
tui-settings-backup-keep-config = 내 구성 유지 (복제 모드)
tui-settings-backup-empty = 아직 백업이 없습니다. c를 눌러 생성하세요.
tui-settings-hints-backups = c 생성 · r 복원 · d 삭제 · space 구성 유지 · 1-8 구성 요소
...
tui-settings-models-edit-title = 모델 재정의
tui-settings-models-edit-context-window = 컨텍스트 창
tui-settings-models-edit-max-output = 최대 출력 토큰
tui-settings-models-edit-empty-hint =   필드를 비워두면 해당 재정의를 지우고 카탈로그 값으로 돌아갑니다.
...
tui-agents-detail-owner = { "  " }소유자:{ "    " }
tui-agents-detail-owner-unowned = 소유자 없음
tui-agents-detail-author = { "  " }작성자:{ "    " }
...
tui-memory-config-unsaved = 저장하지 않은 변경 사항 — s를 눌러 저장
tui-memory-config-saving = 저장 중...
tui-memory-config-saved = 저장됨
tui-memory-config-save-failed = 저장할 수 없습니다
tui-memory-config-loading = 설정을 읽는 중...
tui-memory-config-hint =  ↑↓ 이동 · 스페이스 변경 · s 저장 · r 새로고침 · Esc 뒤로
...
tui-goal-run-paused = 목표 실행 일시중지됨: { $id }
tui-goal-run-resumed = 목표 실행 재개됨: { $id }
tui-goal-cadence-updated = 목표 주기 변경됨: { $id }
...
tui-goal-pause-failed = 목표 실행을 일시중지하지 못했습니다
tui-goal-resume-failed = 목표 실행을 재개하지 못했습니다
tui-goal-cadence-failed = 목표 주기를 변경하지 못했습니다
...
tui-goals-hints = { "  " }/ 검색 | n 새로 만들기 | s 시작 | p 일시중지 | x 중지 | c 주기 | d 삭제 | Enter 세부사항 | r 새로고침
tui-goals-none-selected = 선택된 목표가 없습니다.
tui-goals-judge-label = { "  " }목표 심사자:{ " " }
tui-goals-phase-label = { "  " }실행 단계:{ " " }
tui-goals-hint-start = s 실행 시작
tui-goals-hint-running = p 일시중지 | x 중지
tui-goals-hint-paused = s 재개 | x 중지
tui-goals-detail-hints = { "  " }q/Esc 닫기 | { $run_hint } | c 주기 | r 새로고침
tui-goals-run-running = 실행 중
tui-goals-run-paused = 일시중지됨
tui-goals-run-finished = 완료됨
tui-goals-run-maxiter = 최대 반복 횟수 도달
tui-goals-run-ratelimited = 속도 제한됨
tui-goals-run-stopped = 중지됨
tui-goals-resume-from = { "  " }반복 { $iteration }/{ $max }부터 재개
tui-goals-resume-from-simple = { "  " }반복 { $iteration }부터 재개
tui-goals-pause-explainer = 일시중지는 진행 상황을 유지하고, 중지는 폐기합니다.
tui-goals-already-running = 이미 실행 중입니다 — p 일시중지, x 중지.
tui-goals-already-paused = 이미 일시중지 상태입니다 — s 재개, x 폐기.
tui-goals-not-running = 실행 중인 목표만 일시중지할 수 있습니다.
tui-goals-nothing-to-stop = 중지할 실행이 없습니다.
tui-goals-cadence = 주기:
tui-goals-cadence-label = { "  " }주기:{ " " }
tui-goals-cadence-title = 루프 주기
tui-goals-cadence-hint = 에이전트가 다시 시도하는 간격(초)입니다. 비워 두면 기본값 { $default }초를 사용합니다. 주기가 짧을수록 토큰을 더 빨리 소모하며, 최대 { $max }초까지 지정할 수 있습니다.
tui-goals-cadence-secs = { $secs }초
tui-goals-cadence-default = 기본값({ $secs }초)
tui-goals-cadence-nav = Esc 취소 | Enter 저장
tui-goals-cadence-not-a-number = 초 단위 정수를 입력하거나, 기본값을 쓰려면 비워 두세요.
tui-goals-cadence-out-of-range = 주기는 { $min }초에서 { $max }초 사이여야 합니다.
...
# TUI status messages
tui-event-error-prefixed = 오류: { $error }
tui-event-purge-failed = 삭제: { $detail }
tui-event-backup-restored = 복원됨
