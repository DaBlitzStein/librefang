...
agent-set-unknown-field = Unknown field: { $field }. Supported fields: model, temperature, max_tokens, top_p, frequency_penalty, presence_penalty, top_k, min_p, repeat_penalty, context_window, max_output_tokens
...
tui-event-dashboard-status-fetch-failed = Failed to load the daemon status
tui-event-dashboard-audit-fetch-failed = Failed to load recent activity
tui-event-workflows-fetch-failed = Failed to load workflows
tui-event-triggers-fetch-failed = Failed to load triggers
tui-event-sessions-fetch-failed = Failed to load sessions
tui-event-agents-fetch-failed = Failed to load agents
tui-event-kv-fetch-failed = Failed to load memory for agent { $agent_id }
tui-event-clawhub-search-failed = ClawHub search failed
tui-event-clawhub-browse-failed = Failed to browse ClawHub
tui-event-templates-fetch-failed = Failed to load agent templates
tui-event-providers-fetch-failed = Failed to load providers
tui-event-security-fetch-failed = Failed to load security status
tui-event-audit-fetch-failed = Failed to load the audit trail
tui-event-usage-summary-fetch-failed = Failed to load the usage summary
tui-event-usage-by-model-fetch-failed = Failed to load usage by model
tui-event-usage-by-agent-fetch-failed = Failed to load usage by agent
tui-event-tools-fetch-failed = Failed to load tools
tui-event-groups-fetch-failed = Failed to load groups
tui-event-peers-fetch-failed = Failed to load peers
tui-event-logs-fetch-failed = Failed to load logs
tui-event-goals-fetch-failed = Failed to load goals
tui-event-hands-fetch-failed = Failed to load hands
tui-event-active-hands-fetch-failed = Failed to load active hands
tui-event-extensions-fetch-failed = Failed to load extensions
tui-event-extension-health-fetch-failed = Failed to load extension health
tui-event-comms-topology-fetch-failed = Failed to load the agent topology
tui-event-comms-events-fetch-failed = Failed to load communication events
tui-event-agent-model-label-fetch-failed = Failed to load the model for agent { $agent_id }
...
tui-mod-vault-key-saved = Stored { $key }
tui-mod-vault-key-deleted = Cleared { $key }
tui-mod-vault-key-saved-env-override = Stored { $key }, but the daemon reads it from its own environment — the stored value is not the one it uses
tui-mod-vault-key-deleted-env-override = Cleared { $key } from the vault, but the daemon still reads it from its own environment — nothing was revoked
...
tui-templates-hints =   [↑↓] Navigate  [Enter] Spawn Agent  [f] Filter Category  [p] Promote  [r] Refresh  [R] Restore  [v] History
...
tui-templates-restore-invalid-name = Invalid template name
...
tui-templates-restore-failed = Could not restore from registry
tui-templates-history-failed = Could not load version history
tui-templates-history-loading = Loading version history for { $name }…
tui-templates-history-daemon-only = Version history requires daemon mode
tui-templates-history-title = Version History for { $name }
tui-templates-history-col-id = Version ID
tui-templates-history-col-created = Created
tui-templates-history-col-source = Source
tui-templates-history-error = Version history could not be loaded: { $detail }
tui-templates-history-shape = The daemon's version-history response has an unexpected shape
tui-templates-history-empty = No version history available
tui-templates-history-manifest-only = Version history is only available for custom templates
tui-templates-version-restoring = Restoring { $name } to the selected version…
tui-templates-version-restore-failed = Could not restore version
tui-templates-history-hints = [Esc] Back  [↑↓] Navigate  [Enter] Restore
...
tui-settings-tab-vault = 7 Vault
tui-settings-hints-vault =   [↑↓] Navigate  [e] Set value  [d] Clear  [r] Refresh
tui-settings-vault-header-key = KEY
tui-settings-vault-header-status = STATUS
tui-settings-vault-loading = Loading vault keys...
tui-settings-vault-empty = This daemon exposes no writable vault keys
tui-settings-vault-status-set = Set
tui-settings-vault-status-notset = Not set
tui-settings-vault-status-env = Overridden by the environment
tui-settings-vault-input-prompt = Enter the new value for { $key }
tui-settings-vault-note = Stored values are never shown again. [e] replaces, [d] clears.
tui-settings-vault-note-env-override = { $keys } comes from the daemon's own environment, which overrides the vault. [e] and [d] change the stored copy, not the credential the daemon uses.
tui-settings-vault-delete-confirm =   Clear this vault key? [y] Yes  [any] Cancel
...
tui-agents-workspaces-help = a add · d delete · Enter edit · Tab/Shift+Tab field · s save · Esc back
...
tui-event-workspaces-not-available-in-process = Shared folders editing needs a running daemon.
...
tui-workflows-hints-runs =   [↑↓] Navigate  [a] Auto-refresh  [r] Refresh  [Esc] Back
...
tui-workflows-hint-steps = Agent source picks what the value names: a running agent's ID, an agent's name, or an agent type, which names a template and is spawned when nothing of that type is running. Session "New per run" gives the step a fresh session on every run.
tui-workflows-label-steps = Steps:
tui-workflows-steps-empty = No steps yet. Press [a] to add one.
tui-workflows-step-field-name = Step name
tui-workflows-step-field-source = Agent source
tui-workflows-step-field-prompt = Prompt
tui-workflows-step-field-session = Session
tui-workflows-source-id = Agent ID
tui-workflows-source-name = Agent name
tui-workflows-source-type = Agent type
tui-workflows-session-persistent = Persistent
tui-workflows-session-new = New per run
tui-workflows-placeholder-agent-id = UUID of a running agent
tui-workflows-placeholder-agent-name = name of an existing agent
tui-workflows-placeholder-agent-type = template name, spawned if none is running
tui-workflows-placeholder-prompt = what this step asks; {"{{input}}"} is the workflow input
tui-workflows-steps-none = Add at least one step with [a].
tui-workflows-step-agent-required = Step { $step }: { $source } is required
tui-workflows-step-prompt-required = Step { $step }: Prompt is required
tui-workflows-label-review = Review — press Enter to create
tui-workflows-review-name =   Name:  
tui-workflows-review-desc =   Desc:  
tui-workflows-review-steps =   Steps:  
tui-workflows-hints-create-submit =   [Enter] Create  [Esc] Back
tui-workflows-hints-create-next =   [Enter] Next  [Esc] Back
tui-workflows-hints-create-steps-list =   [a] Add  [d] Delete  [↑↓] Select step  [Tab] Fields  [Enter] Next  [Esc] Back
tui-workflows-hints-create-steps-text =   [Tab/Shift-Tab] Fields  [↑↓] Select step  [Enter] Next  [Esc] Back
tui-workflows-hints-create-steps-select =   [Space/←→] Change  [Tab/Shift-Tab] Fields  [↑↓] Select step  [Enter] Next  [Esc] Back
...
tui-agents-param-top-k-hint = top_k — consider only the K likeliest tokens. Anthropic, Gemini, Ollama, OpenRouter and local servers only.
tui-agents-param-min-p = Minimum probability
tui-agents-param-min-p-hint = min_p — drop tokens far less likely than the top one. Ollama, vLLM, llama.cpp and OpenRouter only.
tui-agents-param-repeat-penalty = Repetition penalty
tui-agents-param-repeat-penalty-hint = repeat_penalty — discourages recently repeated tokens; 1.0 is off. Ollama, OpenRouter and local servers only.
...
tui-agents-label-routing-stable-inert = The kernel runs in Stable mode: no router runs and this agent uses { $model }. Settings saved here take effect once the kernel leaves Stable mode.
tui-agents-label-routing-stable-inert-manifest = The kernel runs in Stable mode: no router runs and this agent uses its manifest model. Settings saved here take effect once the kernel leaves Stable mode.
...
tui-goals-label-loop-engineering = Loop engineering:
tui-goals-label-verifier = Verifier agent:
tui-goals-label-evaluator = Evaluator model:
tui-goals-label-verify-rounds = Verification rounds:
tui-goals-loop-engineering-on = enabled
tui-goals-loop-engineering-off = disabled
tui-goals-verifier-none = none
tui-goals-evaluator-none = none, the agent's own marker decides
tui-goals-hint-stop = [s] stop run
tui-goals-hint-start = [s] start run
tui-goals-hint-rounds = [+/-] Rounds (verify_max_retries)
tui-goals-detail-hints = [q/Esc] Close { $run_hint } [r] Refresh
tui-goals-verify-rounds-default = default
tui-goals-verify-rounds-next = { $current } (next: { $next })
...
tui-goals-loop-engineering-hint = Gate each iteration on a second agent's verdict instead of the agent's own claim of completion. Off reproduces the plain goal loop exactly. [space] toggles it, [y]/[n] set it. Field: loop_engineering.
tui-goals-verifier-hint = UUID of the agent that grades the work. Leave empty for no verifier gate. An agent does not grade its own work. Field: verify_agent_id.
tui-goals-evaluator-hint = Model that judges whether the goal is met, independently of the agent's GOAL_DONE marker. A small, cheap model is the intended choice. Leave empty to rely on the marker alone. Field: evaluator_model.
...
tui-goals-label-tick = Tick interval:
tui-goals-tick-hint = How long the runner pauses between loop iterations, in seconds (tick_interval_secs). Leave empty for the default of { $default }. Accepted range { $min }–{ $max }.
tui-goals-tick-value = { $secs }s
tui-goals-tick-default = { $secs }s (default)
tui-goals-tick-out-of-range = Tick interval must be between { $min } and { $max } seconds.
...
agent-routing-label-stable = Stable mode
agent-routing-stable-inert = no router runs; this agent uses { $model } until the kernel leaves Stable mode
agent-routing-stable-inert-manifest = no router runs; this agent uses its manifest model until the kernel leaves Stable mode
