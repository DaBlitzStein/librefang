...
chat-runner-goal-no-agent = 未选择智能体。
...
chat-runner-no-agent-templates = 未找到智能体类型。请运行 `librefang init`。
chat-runner-invalid-template = 智能体类型 '{ $name }' 无效: { $error }
...
tui-event-model-routing-update-error = 模型路由更新: { $error }
...
tui-event-model-overrides-fetch-failed = 加载 { $model } 的覆盖设置失败
tui-event-model-overrides-save-failed = 保存 { $model } 的覆盖设置失败
tui-event-model-overrides-reset-failed = 重置 { $model } 的覆盖设置失败
...
tui-mod-model-overrides-saved = { $model } 的覆盖设置已保存
tui-mod-model-overrides-reset = { $model } 的覆盖设置已重置
...
tui-templates-header-template = 类型
...
tui-settings-hints-models =   [↑↓] 导航  [e] 编辑窗口  [x] 重置  [r] 刷新
tui-settings-hints-models-edit =   [Tab] 字段  [Enter] 保存  [Esc] 取消
tui-settings-hints-tools =   [↑↓] 导航  [r] 刷新
tui-settings-tab-providers = 1 提供商
tui-settings-tab-models = 2 模型
tui-settings-tab-tools = 3 工具
tui-settings-tab-backups = 5 备份
tui-settings-tab-capabilities = 4 能力
tui-settings-hints-capabilities =   [↑↓] 导航  [Enter] 编辑  [r] 刷新
tui-settings-hints-capabilities-edit =   输入 提供商/模型  [Enter] 保存  [Esc] 取消（留空则自动检测）
tui-settings-capabilities-header-capability = 能力
tui-settings-capabilities-header-target = 提供商 / 模型
tui-settings-capabilities-auto = 自动检测
tui-settings-capabilities-explainer = 所有智能体的全局默认值；智能体自身的 [capabilities] 会覆盖它。
tui-settings-capability-image-understanding = 视觉
tui-settings-capability-speech-to-text = 语音转写
tui-settings-capability-image-generation = 图像生成
tui-settings-capability-text-to-speech = 语音合成
tui-settings-capability-video-generation = 视频生成
tui-settings-capability-music-generation = 音乐生成
tui-event-capabilities-fetch-failed = 加载能力路由失败
tui-event-capabilities-save-failed = 保存 { $capability } 的路由失败
tui-mod-capability-routing-saved = 已保存 { $capability } 的路由
tui-settings-backup-keep-config = 保留我的配置（克隆模式）
tui-settings-backup-empty = 还没有备份。按 c 创建一个。
tui-settings-hints-backups = c 创建 · r 恢复 · d 删除 · space 保留配置 · 1-8 组件
...
tui-settings-models-edit-title = 模型覆盖设置
tui-settings-models-edit-context-window = 上下文窗口
tui-settings-models-edit-max-output = 最大输出 Token
tui-settings-models-edit-empty-hint =   留空该字段以清除该覆盖值并回退到目录值。
...
tui-agents-detail-owner = { "  " }所有者:{ "   " }
tui-agents-detail-owner-unowned = 无所有者
tui-agents-detail-author = { "  " }作者:{ "     " }
...
tui-memory-config-unsaved = 有未保存的更改 — 按 s 保存
tui-memory-config-saving = 正在保存...
tui-memory-config-saved = 已保存
tui-memory-config-save-failed = 无法保存
tui-memory-config-loading = 正在读取配置...
tui-memory-config-hint =  ↑↓ 移动 · 空格 修改 · s 保存 · r 刷新 · Esc 返回
...
model-overrides-saved = { $model } 的覆盖设置已保存
model-overrides-save-failed = 保存 { $model } 的覆盖设置失败(状态 { $status })
model-overrides-cleared = { $model } 的覆盖设置已清除
model-overrides-clear-failed = 清除 { $model } 的覆盖设置失败(状态 { $status })
model-overrides-none-set = { $model } 未设置任何覆盖。
...
tui-goal-run-paused = 目标运行已暂停: { $id }
tui-goal-run-resumed = 目标运行已恢复: { $id }
tui-goal-cadence-updated = 目标节奏已更新: { $id }
...
tui-goal-pause-failed = 暂停目标失败
tui-goal-resume-failed = 恢复目标失败
tui-goal-cadence-failed = 更新目标节奏失败
...
tui-goals-hints = { "  " }/ 搜索 | n 新建 | s 启动 | p 暂停 | x 停止 | c 节奏 | d 删除 | Enter 详情 | r 刷新
tui-goals-none-selected = 未选择目标。
tui-goals-judge-label = { "  " }目标裁判:{ " " }
tui-goals-phase-label = { "  " }运行阶段:{ " " }
tui-goals-hint-start = s 启动运行
tui-goals-hint-running = p 暂停 | x 停止
tui-goals-hint-paused = s 恢复 | x 停止
tui-goals-detail-hints = { "  " }q/Esc 关闭 | { $run_hint } | c 节奏 | r 刷新
tui-goals-run-running = 运行中
tui-goals-run-paused = 已暂停
tui-goals-run-finished = 已完成
tui-goals-run-maxiter = 已达最大迭代次数
tui-goals-run-ratelimited = 已限流
tui-goals-run-stopped = 已停止
tui-goals-resume-from = { "  " }将从第 { $iteration }/{ $max } 次迭代恢复
tui-goals-resume-from-simple = { "  " }将从第 { $iteration } 次迭代恢复
tui-goals-pause-explainer = 暂停会保留当前进度，停止则会丢弃。
tui-goals-already-running = 该目标已在运行 — p 暂停，x 停止。
tui-goals-already-paused = 该目标已暂停 — s 恢复，x 丢弃。
tui-goals-not-running = 只有正在运行的目标才能暂停。
tui-goals-nothing-to-stop = 该目标没有可停止的运行。
tui-goals-cadence = 节奏:
tui-goals-cadence-label = { "  " }节奏:{ " " }
tui-goals-cadence-title = 循环节奏
tui-goals-cadence-hint = 代理每隔多少秒再试一次。留空则使用默认值 { $default } 秒。节奏越短，消耗令牌越快；最长可设 { $max } 秒。
tui-goals-cadence-secs = { $secs } 秒
tui-goals-cadence-default = 默认（{ $secs } 秒）
tui-goals-cadence-nav = Esc 取消 | Enter 保存
tui-goals-cadence-not-a-number = 请输入整数秒，或留空以使用默认值。
tui-goals-cadence-out-of-range = 节奏必须介于 { $min } 秒到 { $max } 秒之间。
tui-goals-new-title = 新建目标
tui-goals-step = { "  " }步骤 { $n }/{ $total }
