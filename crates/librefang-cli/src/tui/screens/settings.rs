...
// `Debug` is load-bearing: the capability tests below assert on the action a
// keypress produced and name the unexpected one in the panic message.
#[derive(Debug)]
...
impl SettingsState {
    ...
    pub fn handle_key(&mut self, key: KeyEvent) -> SettingsAction {
        ...
        // the digits belong to the text field, not to the tab bar.
        ...
        if self.restore.is_none() && !self.confirm_delete {
            match key.code {
                ...
                KeyCode::Char('5') => {
                    self.switch_sub(SettingsSub::Capabilities);
...
mod capability_tests {
    ...
    #[test]
    fn tab_5_opens_capabilities_and_requests_a_refresh() {
        // `4` stayed with Backups through the merge, so Capabilities took the
        // next free digit rather than displacing a binding operators already
