...
mod tests {
    ...
    fn a_list_reload_keeps_run_state_already_fetched_so_pause_stays_live() {
        ...
        s.apply_run_state("1", Some("paused".to_string()), Some(3), Some(25), Some(6));
        ...
        // The budget is the fourth carried field, and the one a reload landing
        // second used to blank: `adjust_verify_max_retries` then fell back to
        // the compiled default rather than the budget the run is under, so the
        // number the operator was looking at moved under them.
        assert_eq!(s.goals[0].run_verify_max_retries, Some(6));
