                        ...
                        .filter_map(|p| {
                            ...
                            Some(crate::tui::screens::workflows::WorkflowParamField {
                                ...
                                description: p
                                    ...
                                    .unwrap_or("")
                                    .to_string(),
                                // Seed the declared default so the operator
                                // only types what differs from it.
                                value: p
                                    .get("default")
...
pub fn spawn_update_agent_workspaces(
    ...
    std::thread::spawn(move || {
        if let BackendRef::Daemon { base_url, api_key } = backend {
            let client = make_daemon_client(api_key.as_deref());
            // Full-manifest replacement: fetch, patch [workspaces], send back.
            let patched = client
                .get(format!("{base_url}/api/agents/{agent_id}/manifest"))
                .send()
                .ok()
...
fn local_agent_templates() -> Vec<TemplateInfo> {
    ...
    for entry in entries.flatten() {
        ...
        if !manifest_path.is_file()
            || manifest_path.extension().and_then(|e| e.to_str()) != Some("toml")
        {
            continue;
        }
        // The file stem is the identity `/api/agent-types/{name}/toml` resolves by, so it — not the manifest's own `name` field — is what a row must carry, or spawning it 404s.
