...
pub(crate) fn cmd_audit_reanchor(config: Option<PathBuf>, confirm: bool, note: &str) {
    ...
    let break_point = match librefang_runtime_audit::diagnose_chain(&conn) {
        ...
        Err(e) => {
            ui::error(&i18n::t_args("monitoring-audit-reanchor-diagnose-failed", &[("error", &e)]));
            std::process::exit(1);
        }
    };
    ...
    ui::kv(&i18n::t("monitoring-audit-reanchor-expected"), &break_point.expected);
    ui::kv(&i18n::t("monitoring-audit-reanchor-found"), &break_point.found);
    ...
    if !confirm {
        ...
        println!(
            ...
            i18n::t_args(
                ...
                &[("seq", &break_point.seq.to_string()), ("path", &db_path.display().to_string())]
            )
        );
    ...
    let outcome = match librefang_runtime_audit::reanchor_after_break(&mut conn, &break_point, note) {
        Ok(o) => o,
        Err(e) => {
            ui::error(&i18n::t_args("monitoring-audit-reanchor-failed", &[("error", &e)]));
            std::process::exit(1);
        }
    };
    drop(conn);
