...
impl ResolvedInferenceParams {
    ...
    /// Any copy of those keys — or of the `top_k` / `min_p` / `repeat_penalty` added
    /// beside them — left in `extra_params` is removed, so the map cannot carry a stale
    /// second spelling of a value the typed field already holds.
