...
fn reduce_model(model: ModelConfig) -> ModelConfig {
    // `mode` and `router_override` survive promotion.
    // They select *how* the type picks a model — fixed versus router-driven, which profiles it will accept, what cost tier it will not exceed — which is part of what the type does, the same category as `capabilities.tools`.
    // Neither carries a host path, a credential binding or a private endpoint: `router_override` holds a bool, profile *names*, and a cost tier.
    // A profile name that the importing install has never heard of degrades to the router's own fallback, so publishing one leaks nothing and loses nothing.
    // Stripping them would silently republish a flexible-routing type as a fixed one — a change to its behaviour, not a redaction.
    let ModelConfig {
        mode,
        router_override,
        provider,
