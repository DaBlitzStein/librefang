...
impl AgentTypeSpec {
    ...
    /// Spelling all 59 out turns that into a compile error at the one place where the decision actually has to be made.
    ...
    pub fn into_new_manifest(self, name: String) -> AgentManifest {
        ...
        AgentManifest {
            ...
            model: ModelConfig {
                ...
                // Routing mode and the per-agent router override are deployment policy for the same reason as the sampling knobs above: an agent type that pinned them would decide routing for every spawn of every operator.
            ...
            capabilities: ManifestCapabilities {
                ...
                // Media-capability routing (vision, speech) names concrete providers and models, so it is a per-deployment binding like the endpoint overrides above, not part of the type.
                routing: capability_defaults.routing,
            },

            // Identity and provenance the flat shape does not carry.
            version: crate::VERSION.to_string(),
            author: String::new(),
            // An agent *type* belongs to nobody: ownership is stamped on the deployed agent at spawn time from the principal that asked for it (#7744).
            // Inheriting an owner from the type would hand every agent spawned from it to whoever authored the type.
            owner: None,
