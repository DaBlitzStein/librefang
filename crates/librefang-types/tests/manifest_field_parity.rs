...
fn fully_populated_manifest() -> AgentManifest {
    AgentManifest {
        ...
        author: "houko".into(),
        // `owner` (#7744) is stamped by the server from the authenticated
        // caller, so it is not part of the PATCH edit surface — but it does go
        // through the same `to_string_pretty` write, which is what this fixture
        // guards.
        owner: Some(librefang_types::principal::Principal::User(
            "parity-owner".into(),
        )),
