...
pub fn is_system_channel(channel: &str) -> bool {
    // Delegates rather than re-listing the names, so this and the channels
    // crate cannot disagree — and so the match stays case-insensitive.
    // #8149 fixed a live-web session being told "no live user is watching"
    // because an outer guard lowercased and an inner one did not; an
    // exact-match copy here would reintroduce exactly that split.
