...
use crate::workflow::StepAgentResolution;
...
impl LibreFangKernel {
    ...
    /// `owner` attributes a spawned step agent's spend to the run's owner
    /// (#7717); `None` leaves attribution to the caller's default.
    pub fn resolve_step_agent(
        &self,
        agent_ref: &StepAgent,
        owner: Option<librefang_types::agent::AgentId>,
    ) -> StepAgentResolution {
        match agent_ref {
            StepAgent::ById { id } => self
                .registry_agent_by_id(id)
                .ok_or(crate::workflow::StepAgentError::NotFound),
            StepAgent::ByName { name } => self
                .registry_agent_by_name(name)
                .ok_or(crate::workflow::StepAgentError::NotFound),
            // `fresh` mints a throwaway instance per run, so it cannot go
            // through the find-or-spawn path that deliberately converges on
            // one canonical instance. Ownership is threaded either way.
            StepAgent::ByType {
                template,
                fresh: true,
            } => self.resolve_agent_by_type_or_spawn(template, owner, true),
            StepAgent::ByType {
                template,
                fresh: false,
            } => match self.find_or_spawn_agent_type(template) {
                ...
                Err(e) => {
                    ...
                    // A template that was found but refused (bad manifest, a
                    // rejected spawn, a lost race) is not a missing template.
                    // Reporting it as `NotFound` sends the operator looking
                    // through `agent-types/` for a file that is already there,
                    // so the reason the kernel gave is carried through instead.
                    Err(match e {
                        StepAgentResolveError::Template(_) => {
                            crate::workflow::StepAgentError::NotFound
                        }
                        other => crate::workflow::StepAgentError::SpawnRejected(other.to_string()),
                    })
                }
            },
        }
    ...
    pub fn preview_step_agent(&self, agent_ref: &StepAgent) -> StepAgentResolution {
        match agent_ref {
            StepAgent::ById { id } => self
                .registry_agent_by_id(id)
                .ok_or(crate::workflow::StepAgentError::NotFound),
            StepAgent::ByName { name } => self
                .registry_agent_by_name(name)
                .ok_or(crate::workflow::StepAgentError::NotFound),
            // The preview deliberately ignores `fresh`: a dry run must not
            // report the throwaway instance's random name, because that name
            // does not exist yet and will differ on the real run (#7717).
            // Showing the canonical instance answers the question the preview
            // is actually asked — which type this step binds to.
            StepAgent::ByType { template, .. } => {
                ...
                match load_agent_template(self.home_dir(), template) {
                    Ok((manifest, _)) => {
                        ...
                        Ok((id, template.to_string(), manifest.inherit_parent_context))
                    }
                    Err(e) => {
                        warn!(
                            agent_type = %template,
