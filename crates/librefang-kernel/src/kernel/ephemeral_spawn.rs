...
impl LibreFangKernel {
    /// Refuse an `agent_type` that names *another live agent* rather than a template (#6930).
    ///
    /// [`load_agent_template`] searches `workspaces/agents/<name>/agent.toml`, which is not only where operator-authored templates live — it is also where every running agent's own manifest lives.
    /// Everything else this module hands the worker is pinned to the parent: the tool set, the MCP and skill allowlists, the resource quota, the ledger the spend lands on.
    /// `manifest.model` is the exception, and it is the one that matters most, because `resolve_driver_for_owner` reads `api_key_env` and `base_url` off it: an unrestricted `agent_type` would let any agent that can reach `agent_spawn` run a turn on **another agent's provider credentials, against another agent's endpoint, with another agent's system prompt in context**.
    /// `agent_spawn` takes its `agent_type` straight from model output, so that is a prompt-injectable surface, not merely an operator one.
    ///
    /// Naming *yourself* is always allowed — an agent borrows no identity it does not already hold.
    /// A template that no live agent shadows is allowed, from either search directory, so ordinary agent-type spawns are untouched.
    fn deny_borrowed_agent_manifest(
        &self,
        agent_type: &str,
        loaded_from: &std::path::Path,
        parent_id: AgentId,
    ) -> KernelResult<()> {
        // Only the workspaces directory holds live agents' manifests. A
        // template resolved out of the read-only `registry/agents/` checkout is
        // a template even when a live agent happens to share its name, so
        // checking the name alone would refuse a legitimate spawn.
        let workspaces = self.home_dir().join("workspaces").join("agents");
        if !loaded_from.starts_with(&workspaces) {
            return Ok(());
        }
        match self.agents.registry.find_by_name(agent_type) {
            Some(entry) if entry.id != parent_id => Err(KernelError::LibreFang(
                LibreFangError::CapabilityDenied(format!(
                    "agent_type '{agent_type}' is a live agent's own manifest, not a template — \
                         spawning a worker under another agent's identity, credentials and system \
                         prompt is denied. Publish a template under registry/agents/{agent_type}/ \
                         instead, or spawn the worker under your own agent type."
                )),
            )),
            _ => Ok(()),
        }
    ...
    pub async fn spawn_ephemeral_worker(
        ...
        let (mut manifest, template_tools) = match request.agent_type.as_deref() {
            Some(agent_type) => {
                ...
                self.deny_borrowed_agent_manifest(agent_type, &path, parent_id)?;
        ...
        let mut session = librefang_memory::session::Session {
            ...
            peer_id: None,
            // The worker's session is never persisted, so it has no parent to
            // hang off: `parent_session_id` exists to make a *stored* sub-agent
            // session reachable from the one that spawned it (#7752).
        ...
        let loop_opts = librefang_runtime::agent_loop::LoopOptions {
            is_fork: false,
            // An ephemeral worker has no authenticated human initiator of its
            // own: it runs on behalf of `parent_id`, and that attribution is
            // already carried by the agent identity the run bills to (#7744).
            owner: None,
