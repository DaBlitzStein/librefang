...
impl GoalRunner {
    ...
    pub fn state(&self, goal_id: GoalId) -> Option<GoalRunState> {
        ...
        let now = Utc::now();
        // The pause checkpoint stores loop position only, so the verification
        // configuration is read back off the goal document — the same source
        // `goal_run_resume` reads it from. Reporting `None` here instead would
        // tell an operator inspecting a paused run that it has no verifier,
        // when resuming it will in fact restore one.
        ...
        Some(GoalRunState {
            ...
            // 0 is the "unset" sentinel the runner clamps; a resume takes the
            // default budget because it carries no request body of its own.
