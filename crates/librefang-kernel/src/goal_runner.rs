...
// Upstream's hardcoded `TICK_INTERVAL` (2s) is deliberately not carried over:
// the pause between iterations is per-run configuration here
// (`tick_interval_secs`, clamped by `clamp_goal_tick_interval_secs`), so a
// module-level constant would be dead and would misdescribe the cadence.
