...
impl AgentRegistry {
    ...
    /// (profile allowlist + cost budget). Mirrors `update_web_search_augmentation`.
...
mod tests {
    ...
    // The #7742 tag-projection contract this file used to assert through a
    // dedicated `update_tags(id, tags)` setter is now asserted through
    // `replace_manifest_and_retag`, which is the method the kernel actually
    // calls (`kernel/agent_state.rs`) — see
    // `replace_manifest_and_retag_reprojects_entry_tags_and_index` below. It
    // covers every assertion the old test made (entry.tags, manifest.tags,
    // pruned bucket, added bucket) plus one it did not: an unchanged tag must
    // stay indexed exactly once rather than being duplicated.
