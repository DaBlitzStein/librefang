...
A reload that fails is now remembered against that exact file, and forgotten the moment the file changes, so `librefang vault init` or a rotate-key from another process still gets a real retry on the next call. (#8284) (@DaBlitzStein)
