...
- 从以下位置发现模板：`~/.librefang/workspaces/agents/`（由注册表同步预装，另外还包括通过仪表板、`agent_type_create` 工具或 `save-as-agent-type` 创建的模板）、以及 `LIBREFANG_AGENTS_DIR`（环境变量覆盖）。
- 每个模板是一个 `<name>.toml` 清单文件。
