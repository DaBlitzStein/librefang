...
Endpoints for system health, configuration, agent types, sessions, task queue, backup/restore, migration, and UI bindings.

## Agent Type Endpoints

### GET /api/agent-types

List available agent types from the agents directory. `GET /api/templates` is a deprecated alias of this endpoint — an agent type IS a template, so both spellings resolve to the same handler, but new integrations should use `/api/agent-types`.
...
### GET /api/agent-types/&lbrace;name&rbrace;

Get a specific agent type's manifest and raw TOML. `GET /api/templates/{name}` remains available as a deprecated alias.
