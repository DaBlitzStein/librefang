...
- Templates are discovered from three sources, highest precedence first: `LIBREFANG_AGENTS_DIR` (env override), `~/.librefang/agent-types/<name>.toml` (the canonical store, written by the registry sync, `POST /api/agent-types`, the `agent_type_create` tool and `save-as-agent-type`), and `~/.librefang/workspaces/agents/` (assistant pre-installed, others installed from dashboard).
  The first two match the order the daemon itself resolves a template in; the env var is a CLI-only override the daemon never reads.
