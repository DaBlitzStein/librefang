...
- Templates are discovered from: `~/.librefang/workspaces/agents/` (pre-installed by the registry sync, plus any created via the dashboard, the `agent_type_create` tool, or `save-as-agent-type`), and `LIBREFANG_AGENTS_DIR` (env override).
