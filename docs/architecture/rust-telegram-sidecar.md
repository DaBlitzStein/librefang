...
- **Rich blocks (preferred).** `rich_blocks::markdown_to_blocks` parses the agent's Markdown with `pulldown-cmark` and builds `InputRichBlock` values, so quoted markup arrives as a *string value* rather than as anything Telegram interprets, and a button can only exist as a `RichTextButton` object the converter never constructs.
  ...
  `||spoiler||` and `==highlight==` are Telegram's own Rich Markdown syntax rather than CommonMark, so they are found by scanning assembled inline runs. The pairing rules are Telegram's, read off the live parser by sending each shape as `rich_message.markdown` and diffing the parse it echoes back: an opener may not be followed by a space, a closer may not be preceded by one, the content may not be empty, a pair does not cross a line break, an escaped delimiter is text, and the two nest in either order. Which character was escaped is read off the source offsets `pulldown-cmark` reports, not guessed from the shape of the event stream: an entity or an unresolved `[` splits that stream with no escape in sight, and an escape at the start of a run does not split it at all. "Space" means ASCII — Telegram treats NBSP and the other Unicode spaces as content — and a rejected closer shifts by one character rather than by one delimiter, which is what makes `=====` a highlight over a single `=`.
  The battery of probed shapes is recorded in `src/format/telegram_oracle.json` and diffed by a test, so the claim is checkable rather than asserted: of 165 recorded forms 128 match byte for byte, and the other 37 are listed with a reason in `DIVERGENCES`. Twenty-nine of those are not content at all — Telegram writes the same characters with the same styles down differently, ordering styles that cover identical text its own way, normalising a bare host with a trailing slash, labelling list items, filling in table chrome — and the test *checks* that rather than taking the label's word for it. The remaining eight are named: three entity kinds Telegram detects and this converter does not, a tab it folds into a space, a relative link where GFM and Telegram disagree about what the text even is, a table row where GFM reads the pipes as cell separators, and a backslash before a delimiter, where Telegram's own handling is neither CommonMark's nor self-consistent — it drops the backslash in `\||секрет||` and keeps it in `||секрет\||b||`.
