...
- **Rich blocks (preferred).** `rich_blocks::markdown_to_blocks` parses the agent's Markdown with `pulldown-cmark` and builds `InputRichBlock` values, so quoted markup arrives as a *string value* rather than as anything Telegram interprets, and a button can only exist as a `RichTextButton` object the converter never constructs.
  ...
  The battery of probed shapes is recorded in `src/format/telegram_oracle.json` and diffed by a test, so the claim is checkable rather than asserted: of 196 recorded forms 146 match byte for byte, and the other 50 are listed with a reason in `DIVERGENCES`. Thirty of those are not content at all — Telegram writes the same characters with the same styles down differently, ordering styles that cover identical text its own way, normalising a bare host with a trailing slash, labelling list items, filling in table chrome — and the test *checks* that rather than taking the label's word for it. The remaining twenty are named, and most of them belong to layers other than the scan — including the runs the scan stands down on — Telegram's own HTML subset, its whitespace normalisation, its link and entity detection. In particular: three entity kinds Telegram detects and this converter does not, a tab it folds into a space, a relative link where GFM and Telegram disagree about what the text even is, a table row where GFM reads the pipes as cell separators, a backslash before a delimiter, where Telegram's own handling is neither CommonMark's nor self-consistent — it drops the backslash in `\||секрет||` and keeps it in `||секрет\||b||` — and a named character reference, which Telegram leaves as written while CommonMark decodes it.
  ...
  The scan reads only what the author wrote as text, and it draws that line in two different ways on purpose. A character that arrived as an escape or a reference is marked precisely, so `&#124;&#124;` is not a delimiter and a space spelled `&#32;` is not a space for the flanking rules. A *run* that holds raw inline HTML or an autolink is not scanned at all — bluntly, losing pairs elsewhere in the same paragraph that Telegram would make.
  The blunt half is deliberate and was arrived at the hard way. Marking exactly the non-prose characters put a third meaning into the same table the whitespace and line-break rules read, and a newline inside `<b\nid=x>` stopped being a line break: `||a <b\nid=x> c||` came back with the whole paragraph hidden behind a spoiler where Telegram leaves it literal. A missing spoiler is a formatting loss; a spoiler over text nobody asked to hide is not.
