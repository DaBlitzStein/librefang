...
- **Rich blocks (preferred).** `rich_blocks::markdown_to_blocks` parses the agent's Markdown with `pulldown-cmark` and builds `InputRichBlock` values, so quoted markup arrives as a *string value* rather than as anything Telegram interprets, and a button can only exist as a `RichTextButton` object the converter never constructs.
  ...
  `||spoiler||` and `==highlight==` are Telegram's own Rich Markdown syntax rather than CommonMark, so they are found by scanning assembled inline runs. The pairing rules are Telegram's, read off the live parser by sending each shape as `rich_message.markdown` and diffing the parse it echoes back: an opener may not be followed by a space, a closer may not be preceded by one, the content may not be empty, a pair does not cross a line break, and the two nest in either order — with "space" meaning ASCII, since Telegram treats NBSP and the other Unicode spaces as content. A rejected closer shifts by one character rather than by one delimiter, which is what makes `=====` a highlight over a single `=`.
  The battery of probed shapes is recorded in `src/format/telegram_oracle.json` and diffed by a test, so the claim is checkable rather than asserted: of 102 recorded forms 91 match byte for byte, and the other 11 are listed with a reason in `DIVERGENCES` — Telegram auto-detecting mentions, hashtags and bare URLs, its trailing slash on a bare host, its folding of a tab into a space, a bullet label this converter does not emit, and its own handling of a relative link. None of them is in the delimiter scan.
  Any count like that describes the *sample*, not the converter: the previous version of this text read "22 of 23 probed shapes match exactly", and a wider battery then found five classes of divergence the sample had missed. The number is only worth stating next to the recording it came from.
