...
- **Rich blocks (preferred).** `rich_blocks::markdown_to_blocks` parses the agent's Markdown with `pulldown-cmark` and builds `InputRichBlock` values, so quoted markup arrives as a *string value* rather than as anything Telegram interprets, and a button can only exist as a `RichTextButton` object the converter never constructs.
  ...
  The scan reads only what the author wrote as text. Raw inline HTML, the display text of an autolink and characters that arrived as references are marked as they are built, so a `||` inside an attribute, inside a URL or spelled `&#124;` is not a delimiter — and a space that arrived as `&#32;` is not a space for the flanking rules either. Telegram agrees on all of these: it drops the tag, shows the URL whole and pairs across the reference.
