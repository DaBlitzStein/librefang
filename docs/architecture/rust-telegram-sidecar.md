...
- **Rich blocks (preferred).** `rich_blocks::markdown_to_blocks` parses the agent's Markdown with `pulldown-cmark` and builds `InputRichBlock` values, so quoted markup arrives as a *string value* rather than as anything Telegram interprets, and a button can only exist as a `RichTextButton` object the converter never constructs.
  ...
  `||spoiler||` and `==highlight==` are Telegram's own Rich Markdown syntax rather than CommonMark, so they are found by scanning each assembled inline run — which is also what makes them nest the way Telegram nests them, `**bold ||spoiler||**` being a spoiler inside the bold. The scan cannot reach code: an inline span is a separate node by then, and a fenced block never passes through it.
