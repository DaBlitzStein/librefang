...
- `/goal` now works in the dashboard chat and in the TUI chat, not only in channels (#3355).
  ...
  Serving the list over HTTP opens a window between mount and the first response in which nothing yet knows `/goal` is a command, so the chat holds a slash send until the catalog arrives — the typed text is kept and an inline notice says why — rather than letting it reach the agent as a plain prompt; a failed fetch is deliberately not held, because with no catalog there is no way to tell a builtin from a skill command and skill commands are meant to reach the agent.
