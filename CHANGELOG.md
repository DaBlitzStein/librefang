...
- The providers table now reports what a provider can emit instead of what somebody asked it for.
  ...
  Each catalog entry now records *where* its capacities came from — the operator, the shipped registry, the gateway, or nobody — instead of only whether somebody had asserted them.
  A model discovered from an OpenAI-compatible gateway carries a hardcoded window because that listing shape has no capacity field, and only the first three origins count as a limit, so an invented ceiling is never published, warned against, or clamped to.
  A catalog written before this existed reads back unchanged: an absent origin means the registry, which is what those files hold (#7780).
