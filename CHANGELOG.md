...
- Document the EveryAPI MCP bridge on the MCP/A2A integrations page (EN + zh mirror), and fix the `[[mcp_servers]]` examples that page already carried.
  ...
  The security section states the bypasses rather than implying the globs are a hard block: `require_approval` pauses for a human instead of denying, `trusted_senders` waives it for every MCP tool (`classify_risk` enumerates only built-ins, so all MCP tools are `Low`), a channel `allowed_tools` rule short-circuits ahead of the list, a `hand:`-tagged agent auto-approves everything, `cache_approvals_per_session` defaults to `true` so one approval covers the rest of the session, and per-user RBAC `NeedsApproval` is the only gate all of those respect.
  Three `[[mcp_servers]]` examples per locale predated this and were unparseable — they put `command` / `args` at the top level of the entry and used `env = { … }` as a table, both rejected by `McpServerConfigEntry`'s `deny_unknown_fields` — so anyone copying the neighbouring snippet got a daemon that failed to load its config.
  A new `crates/librefang-extensions/tests/everyapi_catalog_entry.rs` extracts the fenced TOML straight out of the MDX and runs it through the real loader and the production `format_mcp_tool_name`, so the documented catalog entry, config stanza, and approval globs cannot drift from the code (#6583) (@houko)
